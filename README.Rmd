...
labourmarketreports is an R package to produce markdown reports with data from a number of web APIs:
...
## Usage
The main functionality that has been developed so far is to interact with the ONS API, so it's useful to read through the [https://developer.ons.gov.uk/](documentation) before starting to use the package. The main functions are:
...
```
### Helper functions
The above examples only work as the arguments passed in are already known. The package contains a number of helper functions to get the necessary options from the API. (The base URI used above is not taken directly from the API, but from the [https://developer.ons.gov.uk/](documentation).)
